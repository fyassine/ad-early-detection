...
This chapter states which floors and comparators the ablation sequence in
...
\section{Baselines for the Ablation Sequence}\label{sec:baseline-sequence}

Six floors and comparators are read against every configuration described in
\autoref{sec:protocol}. Each is chosen to isolate a specific contrast rather than to
provide a single omnibus comparison.

The first floor uses age and sex alone, with no imaging information. Any configuration that does not
...
temporal-first configuration that also uses a self-supervised encoder isolates the pooling-order
inversion under matched pretraining; comparing the randomly initialised \ac{GELSTM} against
the temporal-first configuration with no pretraining on either side isolates the same inversion with
...
carried through the ablation sequence rather than one.
...
configuration, which is confirmed by copying the authors' released
weights into this implementation parameter for parameter and checking that the two forward
passes agree at that configuration. The released training procedure trains on a single
subject with no held-out split, so training and evaluation for Brain-TokenGT use the same
cross-validation and held-out test protocol applied to every other configuration in this
thesis. This
...
This section reports the reproducibility audit design of the Brain-TokenGT comparator
...
runs without changing any setting, four to seven repeats at each of four seeds. The pooled
held-out test \ac{AUC} reported for this configuration in \autoref{tab:rq3-scorecard}
reflects the resulting run-to-run scatter rather than a single deterministic pass, and the
mechanism consistent with that scatter is discussed below.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{fig8_btgt_reproducibility_audit}
  \caption{Held-out test \ac{AUC} (\ac{DELCODE}-only, 34-subject test set) across repeat
  runs sharing every setting including the seed. Purple: the 19 stabilised Brain-TokenGT
  repeat runs (7 at seed 42, 4 each at seeds 43-45), scattered within every seed. Teal:
  the no-encoder \ac{GELSTM} configuration of \autoref{sec:rq2} -- 5 runs across 3 distinct
  code revisions at seed 42, single runs at seeds 43-45 -- landing on one value per seed.
  The seed-42 Brain-TokenGT range (0.35 \ac{AUC}) is annotated for scale.}
  \label{fig:btgt-reproducibility-audit}
\end{figure}

\subsection{The Trajectory Implementation Used in This Thesis Is Bit-Reproducible}\label{sec:bit-reproducible}

The temporal-first ablation sequence introduced in \autoref{sec:tfgn} runs under the global
...
of the \ac{GELSTM} no-encoder implementation evaluated in this thesis at a fixed seed
reproduces its held-out test \ac{AUC} and cross-validation \ac{AUC} to the last recorded
digit. This was checked twice: once immediately after implementing the configuration, and again after
...
\ac{GELSTM} no-encoder implementation evaluated here under this protocol; it is not stated of
any other configuration or of an inherited pipeline, and it does not extend to the fine-tuned
spatial-first configuration, whose seed-level reproducibility properties are reported alongside the
...
resolvable: \autoref{sec:rq3} reports a matched-window configuration in which the trajectory models
...
pool on the order of eighty training subjects per fold. The \ac{GELSTM} no-encoder configuration processes
...
developed and refined across the ablation sequence described in \autoref{sec:protocol},
