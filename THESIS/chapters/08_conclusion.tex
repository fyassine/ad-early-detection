...
pooled protocol, the selected temporal-first configuration reaches a pooled out-of-fold
...
spatial-first floor by 0.030 \ac{AUC} at 7.6 times fewer trainable parameters. Two
...
so this pattern is not explained by the selected configuration simply carrying more
capacity than its comparators. Under the evaluated \ac{ADNI}-and-\ac{DELCODE} protocol,
temporal-first modelling performs well relative to every spatial-first comparator
evaluated, including the published Brain-TokenGT competitor, which under the evaluated
configuration showed substantially greater run-to-run variability and scored below every
temporal-first configuration that clears the pre-specified floor gates of
