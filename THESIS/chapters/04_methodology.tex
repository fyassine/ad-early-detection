...
where $d = 64$ when an encoder is present and $d = 200$ when the \texttt{none} configuration
...
which selects one of four configurations:
...
The configurations form a chain of contrasts: \texttt{none} versus \texttt{random} isolates the
...
\texttt{pretrained\_finetuned} isolates task adaptation. Under the \texttt{none} configuration, no
...
derived rather than hardcoded. These four configurations apply only to the \ac{GELSTM} and
...
\texttt{none} $\approx$ every other configuration &
...
\texttt{pretrained\_finetuned} $>$ \texttt{pretrained\_frozen} &
  ...
  Switch to finetuned configuration; monitor overfitting \\
\texttt{pretrained\_finetuned} $<$ \texttt{pretrained\_frozen} &
...
controlled by a single \texttt{TFGNTrainConfig} object, so that the entire ablation sequence
...
kept configuration of all preceding configurations and changes exactly one knob from the last
surviving state. A configuration that fails the keep-or-drop criterion has its knob reverted and
the next configuration branches from the last surviving configuration. The two design contrasts of
...
configuration is structurally inert, a property of the architecture under that knob setting.
...
\textbf{SENS} repeats the selected configuration at \texttt{min\_visits=3}, reducing the pool to
...
$0.0145 > 0.0074$, the configuration passes and its knob is carried forward. Had the fourth value
...
\texttt{common/frozen\_read.py} reconstructs each configuration's adapter from its saved summary and
...
evaluation into a selection set. The following table states which configurations were eligible for
which tier.
...
\begin{tabular}{lccc}
\toprule
Configuration & OOF & In-domain test & External (OASIS-3) \\
 & ($n=248$) & ($n=64$) & ($n=60$) \\
\midrule
