...
% Every outcome, verdict and void configuration belongs in Chapter 6.
...
experimental protocol that judges all configurations. \autoref{sec:instrumentation} describes the
...
\times 200}$ is the matrix of node feature vectors. We illustrate the full graph construction pipeline from regional \ac{BOLD} signals to time-conditioned visit graphs in \autoref{fig:graph-construction-pipeline}.

\begin{figure}[p]
  \centering
  \resizebox{0.78\textwidth}{!}{\input{figures/fig_graph_construction_pipeline}}
  \caption{Subject-level functional connectivity graph construction pipeline. (1) Resting-state 4D BOLD fMRI scans are parcellated into $|V| = 200$ cortical ROIs using the Schaefer-200 atlas across seven canonical networks. (2) Regional mean BOLD time series ($\mathbf{T} \in \mathbb{R}^{200 \times L}$) are extracted per parcel. (3) Pairwise Pearson correlation followed by Fisher $z$-transformation yields continuous FC matrix $C \in \mathbb{R}^{200 \times 200}$. (4a) Connectivity rows form node feature vectors ($X = C$). (4b) Symmetrised $k$-NN sparsification over $|C|$ (pretraining $k=16$) yields binary adjacency $A$. Both components combine into visit-level attributed graphs $\mathcal{G}^{(t)} = (V, A^{(t)}, X^{(t)})$. (5) Sequential visit graphs are paired with elapsed interval timing $\Delta t$. Inset panels are synthetic schematic illustrations of the construction steps.}
  \label{fig:graph-construction-pipeline}
\end{figure}

The prediction target is a single
...
pairwise coupling with every other region. The input feature dimensionality is consequently 200.
...
\]
where $\text{cum}_t$ is the cumulative months elapsed since baseline and the normaliser corresponds 
to nine years, the longest observed follow-up in the pooled cohort. 
This representation directly addresses the irregular
...
between-group variation at the available sample sizes, as \autoref{sec:group-stats}
...
The \ac{GEC} model implements embed-then-classify: the frozen \ac{GAAE} encoder is
...
classification logit. An attention-pooling variant replaces mean pooling with a learned gate network. The \ac{GEP} model is a multi-visit extension: it encodes each of a subject's visits
...
where $d = 64$ when an encoder is present and $d = 200$ when the no-encoder
configuration bypasses the encoder entirely and mean-pools the raw node features. The time-augmented
...
training fold only, which is the recurrent analogue of the per-fold feature
standardisation used by the multilayer perceptron classification heads. Additional
...
\paragraph{The four encoder configurations and the RQ2 ablation.}
The value of reconstruction pretraining is tested by varying one design choice, the
provenance and trainability of the encoder, across four configurations:

\begin{center}
\small
\begin{tabularx}{\textwidth}{@{}lcccX@{}}
\toprule
Configuration & Built? & Loaded? & Trained? & Isolates \\
\midrule
Pretrained, frozen & yes & yes & no & Reference (pre-existing model) \\
Pretrained, fine-tuned & yes & yes & yes & Value of task adaptation \\
Randomly initialised & yes & no & yes & Value of reconstruction pretraining \\
No encoder & no & no & --- & Value of the graph encoder itself \\
\bottomrule
\end{tabularx}
\end{center}

The configurations form a chain of contrasts: no encoder versus randomly initialised
isolates the encoder architecture, randomly initialised versus pretrained frozen isolates
reconstruction pretraining, and pretrained frozen versus pretrained fine-tuned isolates
task adaptation. Under the no-encoder configuration no encoder module is constructed and
the raw node features are mean-pooled directly, so the per-visit embedding width becomes
200 and the recurrent input width follows from the data rather than from a fixed
setting. These four configurations apply only to the \ac{GELSTM} and
\ac{GEGRU} models, because the \ac{GEC} and \ac{GEP} models pre-compute embeddings
offline with a frozen encoder; end-to-end encoder training has no meaning for them without
restructuring those models.

\paragraph{Pre-specified interpretation table.}
The following interpretation table was written before any configuration was run, so that the reading
...
\midrule
Randomly initialised $\approx$ pretrained frozen \newline $\approx$ pretrained fine-tuned &
  Reconstruction adds nothing beyond architecture prior &
  Keep as compute-saving initialisation; report as null finding \\[3pt]
No encoder $\approx$ every other configuration &
  Graph encoder not earning its place &
  Focus on the longitudinal head \\[3pt]
Both pretrained configurations $>$ randomly initialised, \newline both above no encoder &
  Pretraining contributes real transferable structure &
  Keep the \ac{GAAE} stage; quantify the gain \\[3pt]
Pretrained fine-tuned $>$ pretrained frozen &
  Pretrained features under-adapted to conversion task &
  Switch to the fine-tuned configuration; monitor overfitting \\[3pt]
Pretrained fine-tuned $<$ pretrained frozen &
  Fine-tuning overfits the small converter cohort &
  Justify freezing explicitly as regularisation choice \\[3pt]
Randomly initialised $>$ both pretrained configurations &
  Reconstruction pretraining actively harmful &
  Investigate pretext--downstream mismatch \\[3pt]
All four configurations $\approx$ metadata floor &
...
ablation is that it applies only to the \ac{GELSTM} and \ac{GEGRU} models; the
...
In this section we state the hypothesis the temporal-first design tests, describe the
architecture and the modules that the ablation sequence switches on and off, give the
composite training objective, record the design decisions fixed before any run, and
account for the resulting model capacity.

\subsection{Hypothesis, Inputs and Switchable Stages}\label{sec:tfgn-architecture}
...
improve prediction at matched or lower capacity. We contrast the dataflow and inductive biases of both paradigms in \autoref{fig:spatial-vs-temporal-paradigm}.

\begin{sidewaysfigure}
  \centering
  \resizebox{0.92\linewidth}{!}{\input{figures/fig_spatial_vs_temporal_schematic}}
  \caption{Architectural comparison between spatial-first and temporal-first longitudinal graph modelling paradigms. (a) In the spatial-first paradigm (\ac{GELSTM} and \ac{GEGRU}), each multi-region brain graph $\mathcal{G}_t$ is first compressed into a low-dimensional visit embedding $\mathbf{z}_t \in \mathbb{R}^{64}$ by a spatial graph encoder ($f_\theta$), collapsing fine-grained regional identities prior to sequence-level temporal modelling. (b) In the proposed temporal-first paradigm (\ac{TFGN}), a node-shared recurrent core ($g_\phi$) processes each brain region's longitudinal feature trajectory independently into regional representations $\mathbf{h}_v \in \mathbb{R}^{64}$, preserving region-resolved trajectory dynamics until whole-brain spatial aggregation.}
  \label{fig:spatial-vs-temporal-paradigm}
\end{sidewaysfigure}
...
and, for configurations that use them, the per-subject binary connectivity-change mask $M$, the
...
controlled by a single training-configuration object, so that the entire ablation
sequence corresponds to toggling switches on one model. \autoref{fig:tfgn-architecture} details the tensor flow across all seven modules, highlighting the core S1 pathway alongside switchable ablation components and auxiliary loss constraints.

\begin{sidewaysfigure}
  \centering
  \resizebox{0.92\linewidth}{!}{\input{figures/fig_tfgn_architecture}}
  \caption{Detailed architectural block diagram of the proposed \ac{TFGN}. The pipeline processes longitudinal regional connectivity trajectories ($X \in \mathbb{R}^{T \times 200 \times 200}$) with interval timing $\log(1+\Delta t)$ via a node-shared \ac{LSTM} (1). Regional hidden states are optionally modulated by a temporal saliency gate (2), fused with \ac{GVAE} graph autoencoder embeddings over baseline $A_0$ (3, 4), and aggregated across nodes via spatial mean or sparsemax pooling (5). The pooled patient embedding feeds the primary classification head alongside optional topological saliency scoring (6) and a gradient-reversal cohort-adversary head (7). Solid blue boxes denote the core S1 configuration; dashed orange boxes indicate switchable ablation modules.}
  \label{fig:tfgn-architecture}
\end{sidewaysfigure}
...
of $\mu$ on $[\text{age},\, \text{sex}]$ is applied before reparameterised sampling. When no reconstruction target is set, this stage is absent entirely.

\textbf{(4) Fusion.} When a \ac{GVAE} is present, the temporal hidden state $h_i$ and
the graph latent $z_i$ are combined by either
$\operatorname{LayerNorm}(W_u[h_i \,\|\, z_i])$, the concatenate-and-project fusion, or by
passing $z_i$ alone, the latent-only fusion.
...
\textbf{(7) Cohort-adversary head.} Used only by the escalation configuration. A gradient-reversal
...
\ac{ADNI}-versus-\ac{DELCODE} classification head, training the encoder to be uninformative about
cohort identity.

\subsection{The Composite Training Objective}\label{sec:tfgn-loss-section}
...
\subsection{Pre-Specified Design Decisions}\label{sec:tfgn-decisions}
...
$\Delta A \in [-2,2]$. The pre-specified default reconstruction target is the top-$k$ connectivity-change
mask: a per-subject binary change mask $M_{ij} = \mathbb{1}[|\Delta
...
configuration, the pre-specified response is to re-run that configuration with the gradient-reversal
cohort-adversary head and report both alongside each other. The outcome of this check is
reported in \autoref{sec:rq4}.

\subsection{Pretraining and Capacity Accounting}\label{sec:tfgn-pretraining}
...
downstream configurations using this checkpoint remain comparable to configurations using the \ac{DELCODE}-only
checkpoint.
...
re-thresholding. The inherited default for threshold selection is Youden's J, but every experiment
reported in this thesis uses the best-F1 selection, which is the protocol default here.
...
more than one fold. The per-fold feature standardisation for embeddings is fitted on the
training fold only and applied to validation and test embeddings. The log inter-visit
interval and strength centrality statistics for \ac{TFGN} configurations are fitted on the training
...
20. A plateau-triggered scheduler halves the learning rate when validation loss
...
\paragraph{Ablation sequence as a design.}
The ablation sequence is a sequential architecture search in which each configuration inherits the
kept state of all preceding configurations and changes exactly one architectural
component from the last surviving state. A configuration that fails the keep-or-drop
criterion has that change reverted, and the next configuration branches from the last
surviving state. The two design contrasts of
...
are self-supervised, isolates the inversion against a matched pretraining baseline. We illustrate the overall decision tree, branching configurations, and primary methodological contrasts in \autoref{fig:ablation-sequence-diagram}.

\begin{figure}[p]
  \centering
  \resizebox{!}{0.80\textheight}{\input{figures/fig_ablation_sequence}}
  \caption{Structural overview of the sequential ablation search. Level~0 holds the empirical baselines and screening floors, Level~1 the selected temporal-first model (S1), Level~2 the branching configurations screened under the Tier-2 keep-or-drop rule, and Level~3 the protocol sensitivity and mitigation probes. The dashed connectors mark the two contrasts that isolate the pooling-order inversion without pretraining (S0c versus S1) and under matched self-supervision (S0b versus S1c).}
  \label{fig:ablation-sequence-diagram}
\end{figure}

The ablation configurations are as follows.
...
\textbf{S0d} reproduces the Brain-TokenGT competitor baseline \parencite{dong2023braintokengt}
...
reconstruction target, latent-only fusion and mean readout. The S0c-against-S1
...
\textbf{S3} changes the fusion mode to concatenate-and-project. Under S1's configuration there is no \ac{GVAE} stage and therefore no
graph latent $z_i$ to fuse with the temporal hidden state $h_i$. The change in fusion
mode consequently has no gradient path to any new parameters under this configuration, and
the configuration is structurally inert, which is a property of the architecture under that
setting.
...
\textbf{S5} adds the dual-score head with $\lambda_{\text{cent}} = 0.1$. This configuration is
...
artefact, and the question must not be dropped if the \ac{AUC} result is neutral. The
interpretability validation is described in \autoref{sec:instrumentation}.

\textbf{SENS} repeats the selected configuration with a minimum of three visits per
subject, reducing the pool to 140 subjects. It tests whether longer sequences confer a larger advantage.

\textbf{W3} re-runs the \ac{GELSTM} and the \ac{TFGN} selected configuration with
at most three visits per subject, to create a matched comparison with Brain-TokenGT, which is
architecturally constrained to at most three visits.

\paragraph{The four-tier evaluation protocol.}
This protocol was fixed in its entirety prior to reading any result, structured as a progressive screening funnel with four tiers (floor gates, selection rule, robustness vetoes, and one-pass held-out estimation). Scope is tabulated in \autoref{tab:protocol-scope}.

\textbf{Tier 1 (floor gates).} Every configuration must exceed two pre-computed baselines: the
demographics-only logistic regression (age and sex only), and the static single-visit
classifier evaluated per fold from inside the cross-validation loop itself, so that
neither baseline requires reading the held-out test set.
...
each seed, the mean paired per-fold change in \ac{AUC} is computed (configuration $k$ minus its
parent, matched fold for fold). A configuration is kept if and only if the mean of the four
...
undetectable at this sample size, never that it is harmful; a pass means the configuration is worth
...
tie-breaker applies: among kept configurations, the simplest configuration within one standard error
...
\paragraph{Worked example.}
Consider a configuration evaluated at four seeds, with seed-level mean $\Delta$\ac{AUC} values of
...
$0.0145 > 0.0074$, the configuration passes and its change is carried forward. Had the fourth value
...
\textbf{Tier 3 (robustness vetoes).} After a configuration passes Tier 2, five checks with
pre-specified thresholds are applied. A configuration fails if any of the following conditions
...
60-subject \ac{OASIS} set are read exactly once, on the selected configuration and its pre-specified
secondary configurations, at the out-of-fold-derived threshold with no retraining.
The reading procedure reconstructs each configuration's model from its saved summary
and stored parameters, and refuses to score a split whose results already exist unless an
explicit overwrite is requested, so the one-shot read cannot be spent a second time.

\paragraph{Scope table.}
Spending the one-shot held-out splits on every configuration would turn the external
evaluation into a selection set. \autoref{tab:protocol-scope} states which configurations were eligible for
which tier.

\begin{table}[htbp]
\centering
\small
\caption{Scope of evaluation across model configurations and evaluation tiers.}
\label{tab:protocol-scope}
\begin{tabular}{@{}l ccc@{}}
\toprule
Configuration & Out-of-fold & In-domain test & External \ac{OASIS} \\
 & ($n=248$) & ($n=64$) & ($n=60$) \\
\midrule
S0-demo, S0a, S0b, S0c & yes & no & no \\
S0d Brain-TokenGT & yes & post hoc & post hoc \\
\addlinespace[3pt]
...
normalisation $\hat{w} = \tilde{w}/\|\tilde{w}\|_2$ is the conversion axis. The per-scan
conversion score is
\begin{equation}
  s_i = z_i \cdot \hat{w},
  \label{eq:conversion-score}
\end{equation}
...
stable-\ac{MCI} side. The conversion score compresses the classifier's linear decision function to a
single scalar per scan. The leakage contract:
the axis is supervised feature selection computed on training data only and is never
re-fitted on test or external data. When evaluating linear decodability via the logistic probe,
cross-validation uses a 5-fold \texttt{StratifiedGroupKFold} grouped strictly by subject identifier,
ensuring that multiple longitudinal visits from the same subject never appear in both training
and validation folds.

\paragraph{The orthogonal-residual trajectory space.}
Subtracting the conversion-axis component from each embedding gives an axis-orthogonal
...
A probe embedding $z_0$ is traversed along the conversion axis in steps:
...
conversion scores across the training set. Each steered point is decoded back to a
...
methodological: a conversion axis that is purely a discriminative artefact with no grounding
...
deduplicated to prevent duplicate entries from violating the binary adjacency
assumption. This tests stability to connectivity graph scaffold changes
...
persisted per run, giving four maps per configuration rather than twenty; this reduces the power of
