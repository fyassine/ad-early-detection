...
\begin{figure}[p]
  ...
  \caption{Subject-level functional connectivity graph construction pipeline. (1) Resting-state four-dimensional \ac{BOLD} \ac{fMRI} scans are parcellated into $|V| = 200$ cortical \acp{ROI} using the Schaefer-200 atlas. (2) Mean BOLD time series ($\mathbf{T} \in \mathbb{R}^{200 \times L}$) are extracted per region. (3) Pairwise Pearson correlation followed by Fisher $z$-transformation yields the continuous functional connectivity matrix $C \in \mathbb{R}^{200 \times 200}$. (4a) Raw connectivity rows form 200-dimensional node feature vectors ($X = C$). (4b) Symmetrised $k$-nearest-neighbour sparsification over $|C|$ constructs the binary adjacency matrix $A$. (5) The resulting visit graph $\mathcal{G}_i^{(t)}$ is paired with normalized interval timing $[\Delta t_t]$ across the longitudinal sequence.}
...
This protocol was fixed in its entirety prior to reading any result.
...
\toprule
Configuration & Out-of-fold ($n=248$) & In-domain test ($n=64$) & External \ac{OASIS} ($n=60$) \\
\midrule
