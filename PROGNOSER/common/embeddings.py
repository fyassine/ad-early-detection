...
def find_latest_checkpoint(
    network_combo: str, repo_root: Path | str = REPO_ROOT_DEFAULT
) -> tuple[Path, dict]:
    ...
    if not base.is_dir():
        candidates = sorted(
            (repo_root / "CLASSIFIER" / "notebooks").glob(f"checkpoints_gaae_{network_combo}*")
        )
        if not candidates:
...
def extract_subject_embeddings(
    ...
    print(
        f"[embeddings] {network_combo}/{strategy}: latent={latent_dim}, n_subjects={len(subject_visits)}"
    )

    if strategy == "sequence":
        return _build_sequence_embeddings(
            subject_visits, at_risk_windows, model, device, knn_k, latent_dim
        )
    ...
    if skipped:
        print(
            f'[embeddings] skipped {len(skipped)}: {skipped[:3]}{"..." if len(skipped) > 3 else ""}'
        )

    if strategy == "all_aggs":
        col_names = (
            [f"z_baseline_{i}" for i in range(latent_dim)]
