...
CLASSIFIER_v2/notebooks/ into v2 notebooks.

Applies the following idempotent edits to each target .ipynb:
    * sys.path injection: '.../CLASSIFIER' → '.../CLASSIFIER_v2'
...
Run with:  python CLASSIFIER_v2/scripts/patch_v2_notebooks.py
"""
...
def _patch_sys_path(nb):
    """Replace any 'CLASSIFIER' path in sys.path setup cells with 'CLASSIFIER_v2'."""
    for cell in nb["cells"]:
        ...
        for line in cell["source"]:
            new_line = line.replace(
                ...
                "ad-early-detection/CLASSIFIER_v2'",
            ).replace(
                "ad-early-detection/CLASSIFIER\"",
                "ad-early-detection/CLASSIFIER_v2\"",
            )
...
def _insert_sanity_call(nb, split_dir_subpath):
    ...
    src = [
        ...
        "import sys\n",
        "from pathlib import Path\n",
        "_V2_ROOT = Path('/mnt/e/fyassine/ad-early-detection/CLASSIFIER_v2')\n",
