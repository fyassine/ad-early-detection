...
def _truncate(bundle, n):
    items = [
        {**it, "n_scans": n, "visit_months": it["visit_months"][:n]}
...
def _per_visit_probs(state, item, *, device):
    ...
    return [
        (item["visit_months"][t - 1], 0.1 * item["n_scans"] + 0.01 * t)
...
def test_early_detection_fixed_cohort_holds_cohort_constant():
    ...
    rows = early_detection_fixed_cohort(
        b, _eval_split, _truncate, state_dict=None, threshold=0.5, device="cpu", min_subjects=2
    )
...
def test_early_detection_fixed_cohort_skips_singleclass_tail():
    ...
    b = _bundle([("a", 1, 3), ("b", 0, 3), ("c", 1, 2), ("d", 0, 2), ("e", 1, 5), ("f", 1, 5)])
    rows = early_detection_fixed_cohort(
        b, _eval_split, _truncate, state_dict=None, threshold=0.5, device="cpu", min_subjects=2
    )
...
def _per_visit_probs_decreasing(state, item, *, device):
    ...
    return [
        (item["visit_months"][t - 1], 0.9 - 0.1 * (t - 1)) for t in range(1, item["n_scans"] + 1)
    ]


def test_within_subject_prob_slopes_negative():
    b = _bundle([("a", 0, 4), ("b", 0, 3), ("c", 1, 2)])
    df, stats = within_subject_prob_slopes(
        b, _per_visit_probs_decreasing, state_dict=None, device="cpu"
    )
...
def test_within_subject_prob_slopes_skips_single_visit():
    ...
    df, _ = within_subject_prob_slopes(
        b, _per_visit_probs_decreasing, state_dict=None, device="cpu"
    )
