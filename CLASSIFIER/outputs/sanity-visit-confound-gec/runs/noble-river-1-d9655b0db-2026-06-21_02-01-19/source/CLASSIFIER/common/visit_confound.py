...
def early_detection_fixed_cohort(
    ...
    *,
    ...
    Restrict to subjects with ``n_scans >= min_n_scans`` (default: the max visit
    count), then reuse ``early_detection_table``. Because the cohort no longer
    changes between rows, the AUC trend isolates added-visit information from the
    cohort/class-balance shift of the standard variable-cohort table. Note the
    fixed cohort can be small — interpret with its ``n_subjects``.
    """
