    ...
    *,
    ...
    for s in statuses:
        ...
        if state == "done":
            ...
            colored_state = color(f"{state:<{state_width}}", "white", stream=stream)
        else:
...
def follow_run_log(
    ...
    *,
    ...
    if state == "done":
        ...
        colored_state = color(state, "white", stream=stream)
    elif state == "failed":
        colored_state = color(state, "red", stream=stream)
    else:
        colored_state = state
    ...
    try:
        while True:
            ...
            if current_state in ("done", "failed", "interrupted", "cancelled"):
                if log_file.is_file():
                    try:
                        with open(log_file, "r", encoding="utf-8", errors="replace") as f:
                            f.seek(file_offset)
                ...
                if current_state == "done":
                    ...
                    err = status.get("error") or "error"
                    print(color(f"  ✗ FAILED ({dur_str}) — {err}", "red", stream=stream), file=stream)
                elif current_state in ("interrupted", "cancelled"):
                    print(color(f"  INTERRUPTED ({dur_str})", "white", stream=stream), file=stream)
                stream.flush()
                break
