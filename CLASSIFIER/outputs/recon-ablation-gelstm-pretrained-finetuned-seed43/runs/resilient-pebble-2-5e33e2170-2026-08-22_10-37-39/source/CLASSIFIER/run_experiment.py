...
def run_one(exp: dict, *, no_wandb: bool, require_clean: bool) -> bool:
    ...
    if (run_dir / "run_summary.json").is_file():
        try:
            patch_run_summary(run_dir, {"duration_seconds": round(elapsed, 1)})
        except Exception:
            pass
...
def cmd_follow(exp_id: str, lines: int | None = None) -> None:
    run_dir = find_latest_run(_OUTPUTS, exp_id)
    if run_dir is None:
        print(f"No runs found for experiment {exp_id!r} under {_OUTPUTS / exp_id}.", file=sys.stderr)
        return
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        metavar="EXP_ID",
        dest="follow",
        help="Stream live execution logs and progress for an experiment (e.g. --id <id> --follow or --follow <id>).",
    )
    p.add_argument(
        "--watch",
        "-w",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.follow:
        target_id = args.follow if isinstance(args.follow, str) else args.id
        if not target_id:
            print(
                "Please specify which experiment to follow, e.g. --id <id> --follow or --follow <id>.",
                file=sys.stderr,
            )
            return 2
        cmd_follow(target_id, lines=args.limit)
        return 0
