...
def find_latest_run(outputs_root: str | Path, exp_id: str) -> Path | None:
    """Find the latest run directory for an experiment id."""
    outputs_root = Path(outputs_root)
    ...
    *,
    ...
    for status_path in outputs_root.glob(pattern):
        ...
        if status.get("state") == "running" and status.get("started_at"):
            try:
                st = datetime.fromisoformat(status["started_at"])
                status["duration_seconds"] = round((datetime.now() - st).total_seconds(), 1)
            except Exception:
                pass
        elif "duration_seconds" not in status or status["duration_seconds"] is None:
            dur = infer_run_duration(status_path.parent)
            if dur is not None:
                status["duration_seconds"] = round(float(dur), 1)
