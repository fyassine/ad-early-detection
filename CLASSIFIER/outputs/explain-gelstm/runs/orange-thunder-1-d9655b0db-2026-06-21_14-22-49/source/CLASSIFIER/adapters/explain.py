...
class GAAEExplainAdapter(ExplainAdapter):
    ...
    def diagnostics(self, bundle: Bundle) -> Dict[str, Any]:
        ...
        per_subj, labels = [], []
        for it in bundle.items:
            err = per_node_reconstruction_error(enc, it["graphs"][0], device=self.device)
            per_subj.append(float(err.mean()))
            labels.append(int(it["label"]))
        per_subj = np.array(per_subj); labels = np.array(labels)
...
__all__ = ["get_explain_adapter", "resolve_source_run", "ExplainAdapter",
           "GAAEExplainAdapter", "GECExplainAdapter", "GELSTMExplainAdapter"]
