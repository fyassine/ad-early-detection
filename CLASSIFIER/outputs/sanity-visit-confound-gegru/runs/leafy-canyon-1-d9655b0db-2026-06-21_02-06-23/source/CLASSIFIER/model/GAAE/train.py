    ...
    *,
    ...
    for epoch in outer_bar:
        ...
        for batch in train_loader:
            ...
            combined_dense_adj_reconstructed = torch.zeros(
                (total_nodes, total_nodes), device=device
            )
            start_idx = 0
            for i in range(batch_size_local):
                num_nodes_in_graph = (batch_mask == i).sum().item()
                end_idx = start_idx + num_nodes_in_graph
                combined_dense_adj[start_idx:end_idx, start_idx:end_idx] = dense_adj[
                    i, :num_nodes_in_graph, :num_nodes_in_graph
                ]
                combined_dense_adj_reconstructed[start_idx:end_idx, start_idx:end_idx] = (
                    dense_adj_reconstructed[i, :num_nodes_in_graph, :num_nodes_in_graph]
                )
                start_idx = end_idx
            ...
            loss, _, _ = total_loss_fn(
                ...
                x_reconstructed,
                combined_dense_adj,
                combined_dense_adj_reconstructed,
                mask,
                adj_loss_weight,
            )
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                cond_vec = torch.stack([batch.patient_age, batch.patient_sex.float()], dim=1).to(
                    device
                )
