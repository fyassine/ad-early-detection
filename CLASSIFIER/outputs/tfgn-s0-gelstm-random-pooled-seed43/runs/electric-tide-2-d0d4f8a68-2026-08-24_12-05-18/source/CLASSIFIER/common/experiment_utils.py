...
def collect_results(outputs_root: str | Path) -> List[Dict[str, Any]]:
    ...
    for summary_path in sorted(_iter_run_summaries(outputs_root)):
        ...
        for k, v in metrics.items():
            row[f"metric.{k}"] = v
        # Tier 1-3 OOF evaluation-protocol numbers (common.oof.oof_metrics,
        # written by common.run_artifacts.record_oof_artifacts), when present —
        # DOCS/temporal-first-ablation.md's 2026-08-24 addendum.
        oof = summary.get("oof") or {}
