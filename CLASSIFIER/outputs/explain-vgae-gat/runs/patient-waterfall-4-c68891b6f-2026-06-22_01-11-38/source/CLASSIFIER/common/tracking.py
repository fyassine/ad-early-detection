...
def _build_init_kwargs(
    ...
    # W&B run name = the experiment id (registry-readable) plus the run-runner's
    # generated name (adjective-noun-counter + short commit + timestamp, see
    # run_naming.generate_run_name / run_experiment.run_one) so a run is
    # identifiable both by experiment and by its unique run directory, e.g.
    # ``gelstm-early-detection-first-n-snowy-tide-1-349c3823d-2026-06-20_10-21-16``.
    # Standalone notebook runs (no runner) have no RUN_NAME, so fall back to
    # whichever of the two pieces is set. Folds get a ``-fold{k}`` suffix so
    # they stay distinct within the experiment's group.
    exp_id = exp.get("id")
    run_name_param = params.get("RUN_NAME")
    if exp_id and run_name_param:
        base_name = f"{exp_id}-{run_name_param}"
    else:
        base_name = exp_id or run_name_param or "run"
