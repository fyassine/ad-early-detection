...
def trace_forward(model, data, *, device="cpu") -> Dict[str, Any]:
    ...
    = ``mu`` (the eval-time ``z``, kept for back-compat), ``pooled`` (latent,),
    ``adj_true`` (N, N) the observed adjacency, and ``adj_recon`` (N, N) the
    inner-product adjacency reconstruction (pairs with ``adj_true`` for the
    Stage-5b fidelity comparison, mirroring ``reconstruct_adjacency``), plus an
    ordered ``stages`` list.
