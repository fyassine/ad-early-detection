...
def record_oof_artifacts(
    run_dir: str | Path,
    frame: Any,
    metrics: Dict[str, Any],
) -> Path:
    """Write ``oof_predictions.csv`` and patch ``run_summary.json``'s ``oof`` block.

    ``frame`` is ``common.oof.build_oof_frame()``'s output (a pandas
    ``DataFrame``, typed ``Any`` here to keep this module torch/pandas-import
    free at module load); ``metrics`` is ``common.oof.oof_metrics()``'s
    output. These are the Tier 1-3 numbers
    ``DOCS/temporal-first-ablation.md``'s 2026-08-24 addendum reads instead of
    any test-set number — see ``common.oof`` for why.
    """
    run_dir = Path(run_dir)
    frame.to_csv(run_dir / "oof_predictions.csv", index=False)
    return patch_run_summary(run_dir, {"oof": metrics})
...
def record_external_metrics(
    ...
    *,
    ...
    """Patch ``run_summary.json`` with an external-cohort test result.

    Scored at the in-domain OOF-derived ``threshold`` (never re-optimized on
    the external cohort — see ``.claude/rules/evaluation.md``). Namespaced by
    ``cohort`` (e.g. ``ext_oasis3_auc``) so a run can carry both the in-domain
    ``test_*`` block (from :func:`record_test_metrics`) and one or more
    external blocks without either overwriting the other.

    ``patch_run_summary`` shallow-merges at the top level, so the existing
    ``metrics`` dict is read and updated in place here rather than replaced —
    calling this after :func:`record_test_metrics` must not wipe out
    ``test_auc`` etc.
    """
    run_dir = Path(run_dir)
    existing = json.loads((run_dir / "run_summary.json").read_text())
    merged_metrics = dict(existing.get("metrics") or {})
    prefix = f"ext_{cohort}"
    merged_metrics.update(
        {
            f"{prefix}_auc": float(metrics["auc"]),
            f"{prefix}_f1": float(metrics["f1"]),
            f"{prefix}_sensitivity": float(metrics["sensitivity"]),
            f"{prefix}_specificity": float(metrics["specificity"]),
            f"{prefix}_threshold": float(threshold),
            f"{prefix}_threshold_method": threshold_method,
        }
    )
    return patch_run_summary(
        run_dir,
