...
class LogRegDriftAdapter(LongitudinalAdapter):
    ...
    _FEATURE_SETS = ("drift", "demo")

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.min_visits = self.cfg.get("min_visits")
        self.max_visits = self.cfg.get("max_visits")
        # Tier-1 demographics floor (DOCS/temporal-first-ablation.md's
        # 2026-08-24 addendum §1): "demo" restricts features to [age, sex]
        # only (no PCA, no ΔA) so the arm answers "is there signal beyond
        # age/sex at all?" while riding the identical adapter/CV/threshold/OOF
        # path as every other rung.
        self.feature_set = self.cfg.get("feature_set", "drift")
        if self.feature_set not in self._FEATURE_SETS:
            raise ValueError(
                f"feature_set={self.feature_set!r} invalid; expected one of "
                f"{self._FEATURE_SETS}."
            )
    ...
    def _extract_features(self, items: List[Dict[str, Any]], pca: Optional[PCA] = None):
        ...
        ``feature_set="drift"`` (default): [PCA_32(vec(ΔA)), n_visits,
        total_months, age, sex]. ``feature_set="demo"``: [age, sex] only —
        the Tier-1 demographics floor, no PCA, no ΔA (``pca`` stays ``None``).
        """
        if not items:
            raise ValueError("Empty items list provided to _extract_features")

        if self.feature_set == "demo":
            X = np.stack(
                [[float(item["age"]), float(item["sex"])] for item in items]
            )
            return X, None
    ...
    def model_config(self) -> Dict[str, Any]:
        return {
            ...
            "pca_components": 32 if self.feature_set == "drift" else 0,
            "n_folds": self.n_folds,
            "min_visits": self.min_visits,
            "max_visits": self.max_visits,
        }
    ...
    def load_state(self, run_dir: str) -> Dict[str, Any]:
        """Rebuild ``{pca, scaler, clf, threshold}`` from a run dir.

        ``model_state_for_save`` writes the whole composite ``state`` as
        ``model_state_dict`` — unwrap it the same way every sibling adapter
        does (``gelstm.py`` / ``gec.py``'s ``load_state``), rather than
        returning the outer checkpoint dict itself.
        """
        ckpt = load_run_checkpoint(run_dir, device="cpu")
        return model_state_from_checkpoint(ckpt)
