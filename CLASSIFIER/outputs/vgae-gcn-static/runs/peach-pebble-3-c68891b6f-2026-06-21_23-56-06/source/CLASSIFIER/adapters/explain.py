...
class ExplainAdapter:
    ...
    def supports_region_importance(self) -> bool:
        """Whether :meth:`region_importance` yields a meaningful (non-zero) map.

        True by default — every classifier and the GAAE pool a GAT encoder, which
        always exposes attention weights. The VGAE overrides this: its ``gcn``
        backbone has no attention, so the map would be silently all-zero.
        """
        return True
...
class VGAEExplainAdapter(ExplainAdapter):
    ...
    def capabilities(self) -> set:
        # Attention-based region importance only exists for the GAT backbone — GCNConv
        # has no attention weights, so encode(return_attention=True) returns an empty
        # list and region_importance() would silently report all-zero scores.
        caps = {"reconstruction"}
        if self.cfg.get("conv_type", "gcn") == "gat":
            caps.add("attention")
        return caps

    def supports_region_importance(self) -> bool:
        return self.cfg.get("conv_type", "gcn") == "gat"
