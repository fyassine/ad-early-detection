...
class CVResult:
    ...
    ``oof_folds`` is the 1-based fold number for each row of ``oof_probs`` /
    ``oof_targets`` / ``oof_sids`` (same order). ``oof_extras`` holds any
    additional per-subject OOF arrays a ``fold_probe`` collected (e.g. the
    baseline-scan-only probability for the Tier-1 static-baseline floor — see
    ``common.oof.build_oof_frame``), also in that same row order; empty when
    no ``fold_probe`` was supplied.
    """
...
def run_kfold_cv(
    ...
    *,
    ...
    fold_probe: Optional[Callable[[Bundle, dict], "dict[str, dict[Any, float]]"]] = None,
) -> CVResult:
    ...
    fold_probe : callable, optional
        ``fold_probe(bundle_va, fold_out) -> {extra_name: {subject_id: value}}``,
        called once per fold right after ``train_fold`` returns, with that
        fold's validation ``Bundle`` and its full ``fold_out`` dict (so the
        probe can reuse ``fold_out['state_dict']`` / ``['best_threshold']``).
        Every subject in ``bundle_va`` must have a value for every extra name
        returned (raises otherwise) — see ``common.oof`` for the OOF
        (Tier 1-3 evaluation-protocol) consumer of ``CVResult.oof_extras``.
        ``None`` (default): ``oof_extras`` is empty, everything else unchanged.
    ...
    for fold, (tr_idx, va_idx) in enumerate(
        ...
        fold_sids = list(fold_out["oof_sids"])
        oof_probs.extend(list(fold_out["oof_probs"]))
        oof_targets.extend(list(fold_out["oof_targets"]))
        oof_sids.extend(fold_sids)
        oof_folds.extend([fold + 1] * len(fold_sids))

        if fold_probe is not None:
            bundle_va = bundle.subset(va_idx)
            probe_out = fold_probe(bundle_va, fold_out) or {}
            for name, sid_to_val in probe_out.items():
                missing = [sid for sid in fold_sids if sid not in sid_to_val]
                if missing:
                    raise ValueError(
                        f"fold_probe fold {fold + 1}: extra {name!r} is missing a "
                        f"value for subject_id(s) {missing[:5]}"
                        f"{'...' if len(missing) > 5 else ''} (expected one value "
                        "per oof_sid)."
                    )
                oof_extras.setdefault(name, []).extend(
                    float(sid_to_val[sid]) for sid in fold_sids
                )
    ...
    return CVResult(
        ...
        oof_targets=oof_tgt,
        oof_sids=oof_sids,
        oof_folds=np.asarray(oof_folds, dtype=int),
        oof_extras={k: np.asarray(v, dtype=float) for k, v in oof_extras.items()},
        best_fold=best_fold,
