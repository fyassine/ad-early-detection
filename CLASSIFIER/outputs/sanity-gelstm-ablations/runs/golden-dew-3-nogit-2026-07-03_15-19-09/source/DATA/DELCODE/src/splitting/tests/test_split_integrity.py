"""
Split integrity and leakage tests for DATA/DELCODE/src/splitting/.
...
def test_downstream_n_scans_matches_allowed_months(downstream_splits):
    for split_name, df in downstream_splits.items():
        for _, row in df.iterrows():
            months = [m for m in str(row["allowed_months"]).split(";") if m]
            assert len(months) == row["n_scans"], (
                f"Downstream {split_name}: {row['Pseudonym']} n_scans="
                f"{row['n_scans']} but allowed_months has {len(months)} entries"
            )


def test_downstream_converter_allowed_months_excludes_ad_visits(downstream_splits):
    cohorts = pd.read_csv(COHORTS_CSV)
    for split_name, df in downstream_splits.items():
        converters = df[df["diagnosis"] == "converter"]
        for _, row in converters.iterrows():
            pid = row["Pseudonym"]
            allowed = {int(m) for m in str(row["allowed_months"]).split(";") if m}
            ad_months = set(
                cohorts.loc[
                    (cohorts["Pseudonym"] == pid) & (cohorts["diagnosis"] == "ad"),
                    "visit",
                ]
                .str.replace("M", "", regex=False)
                .astype(int)
            )
            overlap = allowed & ad_months
            assert not overlap, (
                f"Downstream {split_name}: {pid} allowed_months {allowed} includes "
                f"post-conversion 'ad' month(s) {overlap} — label leakage"
            )
