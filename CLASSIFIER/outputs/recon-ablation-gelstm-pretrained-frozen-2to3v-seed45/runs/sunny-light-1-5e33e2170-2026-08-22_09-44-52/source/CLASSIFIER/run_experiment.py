...
def cmd_status(
    *,
    ...
    if watch:
        watch_status_table(
            lambda: read_statuses(_OUTPUTS),
            interval=interval,
            limit=limit,
        )
    else:
        statuses = read_statuses(_OUTPUTS, limit=limit)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Limit status table output to the most recent N runs.",
    )
    p.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.status or args.watch or (
        ...
        cmd_status(watch=args.watch, interval=args.interval, limit=args.limit)
        return 0
    if args.collect:
        cmd_collect()
        return 0
    ...
    if not (args.id or args.all or args.mode):
        print(
            "Nothing to do: pass --id, --all, --mode, --status, --watch, or --collect.",
            file=sys.stderr,
        )
        return 2
