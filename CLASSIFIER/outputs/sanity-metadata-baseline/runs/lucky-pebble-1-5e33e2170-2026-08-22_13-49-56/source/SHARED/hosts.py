...
def gpu_free_mib(host: str, *, timeout: float = 15.0) -> int | None:
    ...
    job fail to start.
    """
    query = "nvidia-smi --query-gpu=memory.total,memory.used --format=csv,noheader,nounits"
    try:
    ...
    if proc.returncode != 0 or not proc.stdout.strip():
        return None
    # First GPU only: both boxes have exactly one.
    first = proc.stdout.strip().splitlines()[0]
    try:
        total_s, used_s = (part.strip() for part in first.split(","))
