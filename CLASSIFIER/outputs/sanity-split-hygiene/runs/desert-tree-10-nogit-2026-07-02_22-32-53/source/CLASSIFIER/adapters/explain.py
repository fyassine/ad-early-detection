...
class GAAEExplainAdapter(ExplainAdapter):
    ...
    def extra(self, name: str, ctx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if name == "disease_axis_steering":
            ...
            return steer_along_axis(
                enc, g, proj["w_hat"], sigma, scales=ctx.get("scales"), device=self.device
            )
            ...
            out.update(
                {
                    ...
                    "feature_fidelity_median": (
                        float(np.nanmedian(feature_fidelity_r))
                        if feature_fidelity_r.size
                        else float("nan")
                    ),
                    "feature_fidelity_iqr": (
                        [
                            float(np.nanpercentile(feature_fidelity_r, 25)),
...
class _ClassifierExplainAdapter(ExplainAdapter):
    ...
    def load(self) -> Dict[str, Any]:
        ...
        ), "read_run_threshold returned None — check the run directory."
        return {
