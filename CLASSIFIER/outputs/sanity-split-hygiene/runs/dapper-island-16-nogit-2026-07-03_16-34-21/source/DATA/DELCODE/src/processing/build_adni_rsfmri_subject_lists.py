...
for desc, grp in fmri_f.groupby("fmri_description")["fmri_group"].first().sort_index().items():
    print(f"    [{grp}]  {desc}")
...
conv_visits = (
    ...
    if conv_visit_rows
    else pd.DataFrame(columns=["PTID", "VISCODE", "EXAMDATE", "DIAGNOSIS", "label"])
)
nonconv_visits = (
...
def attach_fmri(
    visits_df: pd.DataFrame, fmri_df: pd.DataFrame, date_window: int = DATE_WINDOW
) -> pd.DataFrame:
    if visits_df.empty:
    ...
    merged = visits_df.merge(
        fmri_df[FMRI_JOIN_COLS], left_on="PTID", right_on="subject_id", how="left"
    )
    ...
    (set(zip(best["PTID"], best["EXAMDATE"], strict=False)) if len(best) else set())
...
try:
    ...
    print("\n  vs old artifact —")
    print(
        f"    Converters:     {len(old_conv)} → {len(new_conv)} "
        f"(+{len(new_conv - old_conv)} new, -{len(old_conv - new_conv)} dropped)"
    )
    print(
        f"    Non-converters: {len(old_nonconv)} → {len(new_nonconv)} "
