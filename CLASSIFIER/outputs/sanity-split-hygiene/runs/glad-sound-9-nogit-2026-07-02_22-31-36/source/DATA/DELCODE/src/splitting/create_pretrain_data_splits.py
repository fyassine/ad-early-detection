...
DELCODE_DIR = Path(__file__).parents[4] / "DATA" / "DELCODE" / "__fc_wholebrain_sch200_flat__"
MATRICES_DIR = DELCODE_DIR / "matrices"
COHORTS_CSV = Path(__file__).parents[4] / "DATA" / "DELCODE" / "__metadata__" / "cohorts_with_scans.csv"
DOWNSTREAM_SPLITS_DIR = Path(__file__).parents[4] / "DATA" / "DELCODE" / "__metadata__" / "SPLITS" / "downstream"
OUTPUT_DIR = Path(__file__).parents[4] / "DATA" / "DELCODE" / "__metadata__" / "SPLITS" / "pretrain"
...
def main():
    ...
    log(f"Downstream test patients (→ pretrain test): {len(downstream_test_ids)}")
    log(f"Downstream val  patients (→ pretrain val):  {len(downstream_val_ids)}")
    ...
    log(f"Reserved → pretrain val:  {len(downstream_val_reserved)}")
    log(f"Available for free split: {len(available)}")
    ...
    log(f"VAL   : {len(val):4}  {dict(val['diagnosis'].value_counts())}")
    log(f"TEST  : {len(test):4}  {dict(test['diagnosis'].value_counts())}")
