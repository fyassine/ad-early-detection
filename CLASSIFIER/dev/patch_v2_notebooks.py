...
SEEDING_CELL_TAG = "__v2_seeding__"
...
def _insert_seeding(nb):
    """Insert a seeding code cell (idempotent via tag).

    Notebooks that perform any training must seed before constructing
    datasets, samplers, or models. Inserted right after the sys.path cell
    so it runs before any randomness-sensitive code.
    """
    if _has_tag(nb, SEEDING_CELL_TAG):
        return
    src = [
    ...
    cell = _make_code_cell(src, tag=SEEDING_CELL_TAG)
    insert_idx = 1
...
FRAMING = {
    "LONGITUDINAL_GELSTM_DELCODE_WHOLE_BRAIN.ipynb": [
    ...
    "LONGITUDINAL_GELSTM_FDR_FILTERED_DELCODE_WHOLE_BRAIN.ipynb": [
    ...
    "LONGITUDINAL_GEC_MLP_DELCODE.ipynb": [
    ...
    "STATIC_GAAE_LOGREG_DELCODE_WHOLE_BRAIN.ipynb": [
    ...
    "BASELINE_MODEL_COMPARISON_DELCODE_WHOLE_BRAIN.ipynb": [
        "# [v2] Model Comparison\n",
        "\n",
        ...
        "`SANITY_BASELINE_METADATA_TIME.ipynb`) is included as an explicit floor.\n",
    ],
}
...
def patch_one(path: Path):
    ...
    if path.name == "BASELINE_MODEL_COMPARISON_DELCODE_WHOLE_BRAIN.ipynb":
