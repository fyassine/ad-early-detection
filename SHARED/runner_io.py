...
def reconcile_run_status(
    ...
    *,
    ...
    if state == "running":
        ...
        owner = status.get("host")
        if owner and not hosts.is_local(owner):
            # Run belongs to the other box. Its pid means nothing here, so
            # liveness comes from the heartbeat on the shared tree. "unknown"
            # (no heartbeat yet, or an older run that predates them) must be
            # treated as alive: marking a healthy remote run "killed" would
            # corrupt it on disk for every host.
            liveness = hosts.heartbeat_liveness(run_dir)
            alive = liveness != "dead"
...
class RunLifecycle:
    ...
    def __exit__(
        ...
        # Stop first: a heartbeat written after the terminal status would make a
        # finished run look alive to the other box.
        self._stop_heartbeat()
...
def active_runs(
    ...
    *,
    ...
    """Runs that are genuinely executing right now, on EITHER GPU box.

    fritz and frieda share one ``outputs/`` tree over NFS, so this sees both
    boxes' runs without needing ssh: a local run is confirmed against the
    process table, a remote one against the heartbeat it writes to the shared
    tree. Runs whose liveness cannot be established are INCLUDED -- the caller
    uses this to refuse duplicate launches, where a false "not running" is the
    expensive mistake (two hosts racing one ``outputs/<id>/latest`` symlink).
    """
    ...
    for status_path in outputs_root.glob(pattern):
        ...
        if not isinstance(status, dict) or status.get("state") != "running":
            continue
        liveness = hosts.run_liveness(status, status_path.parent, pid_alive=is_process_alive)
        if liveness != "dead":
            status["_path"] = str(status_path)
            status["_liveness"] = liveness
            live.append(status)
    return live


def assert_not_already_running(outputs_root: str | Path, experiment_id: str) -> None:
    """Refuse to start an experiment that is already running on either box.

    ``run_one`` rewrites ``outputs/<id>/latest`` with no locking, so the same id
    started twice -- on one box or across both -- leaves that pointer aimed at
    whichever run happened to touch it last, silently mislabelling which run the
    downstream notebooks read. Fail loudly instead.
    """
    live = active_runs(outputs_root, experiment_id=experiment_id)
    if not live:
        return
    first = live[0]
    where = first.get("host") or "this host"
    detail = f"host={where} pid={first.get('pid')} run={first.get('run_name')}"
    if first.get("_liveness") == "unknown":
        detail += (
            " (no heartbeat yet -- if that run is genuinely dead, delete its "
            "run directory or set its status.json state)"
        )
    raise RuntimeError(
        f"Experiment {experiment_id!r} is already running: {detail}. "
        "Two runs of one id race outputs/<id>/latest. Wait for it, or launch a "
        "different id on the other GPU."
    )
...
def render_status_table(
    ...
    *,
    ...
    # Runs from both GPU boxes share one outputs/ tree, so the table has to say
    # which box each row came from -- otherwise two concurrent runs are
    # indistinguishable. Older runs predate the field and show "-".
    host_names = [str(s.get("host") or "-") for s in statuses]
    host_width = max(len("HOST"), max((len(name) for name in host_names), default=0))
    ...
    for s in statuses:
        ...
        host_str = str(s.get("host") or "-")
        # Highlight the other box so a glance separates local from remote work.
        colored_host = (
            f"{host_str:<{host_width}}"
            if host_str in ("-", hosts.local_host())
            else color(f"{host_str:<{host_width}}", "magenta", stream=stream)
        )
