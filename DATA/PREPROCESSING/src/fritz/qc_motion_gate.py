...
def score_session(
    ...
    *,
    ...
    if n_volumes == 0:
        raise ValueError(
            f"{confounds_path}: 0 volumes left after dropping {dummy} dummy scans."
        )
...
def main() -> None:
    ...
    parser.add_argument("--fmriprep-root", type=Path, required=True,
                        help="dir containing sub-* fMRIPrep output")
    ...
    parser.add_argument("--fd-threshold", type=float, default=MEAN_FD_EXCLUDE_THRESHOLD_MM,
    ...
    parser.add_argument("--dummy", type=int, default=10,
