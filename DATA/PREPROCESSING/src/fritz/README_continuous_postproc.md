...
1. **Gate** — only subjects fMRIPrep finished **cleanly** are eligible,
   detected by the subject-level `sub-<id>.html` report under
   `outputs/<dataset>/fmriprep/` on CORE. fMRIPrep writes that report only at
   the end of a successful subject run, so a subject still being written
   (running) or left partial (timeout/failed) has no report and is skipped.
   This gate is **dataset-scoped and array-agnostic** — it reads only this
   dataset's outputs and unions across the original array and any partial rerun
   arrays, so it needs no SLURM job id. That matters because the job name
   (`fmriprep_oasis_adni`) is shared by both cohorts and reruns land under new
   ids, so no single job id is both correct and cohort-specific. Pass `--jobid`
   only to reproduce a specific run (it intersects the completion set with that
   job's `COMPLETED` tasks).
...
| `--jobid N` | none | Optional. Restrict the gate to subjects whose task `COMPLETED` under this array job (comma-separated list allowed). Omit it — the default gates on the `sub-<id>.html` completion marker across all arrays, which is the correct choice; `--jobid` is only for reproducing one specific run |
