#!/bin/bash
#================================================================
# download_bold_bids.sh
#================================================================
#
# Resumable wrapper around download_oasis_scans_bids.sh for
# downloading OASIS-3 resting-state BOLD fMRI scans in BIDS format.
#
# Features:
#   - Skips sessions already successfully downloaded (non-empty func/ folder)
...
LOG_FILE="${LOG_DIR}/download_bold_${TIMESTAMP}.log"
PROGRESS_FILE="${LOG_DIR}/progress_${TIMESTAMP}.txt"

echo "========================================" | tee -a "$LOG_FILE"
echo "OASIS-3 BOLD BIDS Download" | tee -a "$LOG_FILE"
...
while IFS=, read -r EXPERIMENT_ID; do
    ...
    # Check if sub-OASXXXXX/ses-dXXXX/func/ exists and is non-empty
    ...
    EXPECTED_FUNC="${OUTPUT_DIR}/${BIDS_SUBJ}/${BIDS_SESS}/func"

    if [ -d "$EXPECTED_FUNC" ] && [ -n "$(ls -A "$EXPECTED_FUNC" 2>/dev/null)" ]; then
    ...
    if tar tf "$OUTPUT_DIR/$EXPERIMENT_ID.tar.gz" &> /dev/null; then
        ...
        echo "  [WARN] No valid bold scan found for ${EXPERIMENT_ID}" | tee -a "$LOG_FILE"
        FAILED=$((FAILED + 1))
    fi
...
echo "Failed (no bold scan):  $FAILED" | tee -a "$LOG_FILE"
echo "Output dir: $OUTPUT_DIR" | tee -a "$LOG_FILE"
echo "Log file:   $LOG_FILE" | tee -a "$LOG_FILE"
echo "========================================" | tee -a "$LOG_FILE"
