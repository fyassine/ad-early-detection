...
def small_worldness(adj: np.ndarray, n_random: int = 5, seed: int = 42) -> Optional[float]:
    """
    Humphries & Gurney (2008) σ = (C/C_rand) / (L/L_rand).

    Random reference graphs are generated by edge-preserving rewiring
    (``nx.random_reference``) — slower but produces comparable degree
    distributions. We sample ``n_random`` reference graphs and use the
    median to stabilise the estimate.
    """
    if not _HAS_NX:
        return None
    g = _to_graph(adj)
    try:
        ...
        for _ in range(n_random):
            ...
            try:
                # Connected double-edge swap preserves degree sequence.
                gr = nx.random_reference(g, niter=2, seed=seed_i)
            except Exception:
                # Fallback to Erdos-Renyi with matched p.
                p = (2 * g.number_of_edges()) / (g.number_of_nodes() * (g.number_of_nodes() - 1))
                gr = nx.fast_gnp_random_graph(g.number_of_nodes(), p, seed=seed_i)
