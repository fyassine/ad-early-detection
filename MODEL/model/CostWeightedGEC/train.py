...
def _weighted_bce_with_logits(logits, labels, pos_weight=None, class_cost_weights=None):
    ...
    if class_cost_weights.ndim != 1 or class_cost_weights.numel() != 2:
        raise ValueError("class_cost_weights must be a tensor with shape [2]")

    sample_weights = torch.where(
        labels > 0.5,
        class_cost_weights[1],
        class_cost_weights[0],
    )
    return (base_loss * sample_weights).sum() / sample_weights.sum().clamp_min(1e-12)
...
def train_classifier(model, train_loader, val_loader, optimizer, device, 
                     ...
                     use_scheduler=True, class_cost_weights=None):

    if class_cost_weights is not None and not isinstance(class_cost_weights, torch.Tensor):
        class_cost_weights = torch.tensor(class_cost_weights, dtype=torch.float, device=device)
    elif isinstance(class_cost_weights, torch.Tensor):
        class_cost_weights = class_cost_weights.to(device=device, dtype=torch.float)
    
    wandb.init(project=project_name, config={
        "epochs": epochs,
        "early_stopping_patience": early_stopping_patience,
        "pos_weight": pos_weight.item() if isinstance(pos_weight, torch.Tensor) else pos_weight,
        "class_cost_weights": class_cost_weights.tolist() if isinstance(class_cost_weights, torch.Tensor) else class_cost_weights,
