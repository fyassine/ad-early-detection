...
def train_model_with_val_notebook_train_loss(
    model,
    train_loader,
    val_loader,
    optimizer,
    ...
    dataset_info=None,
    project_name="graph-autoencoder-training",
):
    wandb.init(
    ...
    for epoch in outer_bar:
        ...
        for batch in train_loader:
            ...
            if torch.isnan(x).any():
                print(f"NaN detected in input x, skipping batch")
                continue
            if edge_attr is not None and torch.isnan(edge_attr).any():
                print(f"NaN detected in edge_attr, skipping batch")
                continue

            dense_adj = to_dense_adj(
                edge_index, batch=batch_mask, edge_attr=edge_attr
            ).to(device)
            ...
            if torch.isnan(z).any():
                print(f"NaN detected in latent z at epoch {epoch}")
                continue
            if torch.isnan(x_reconstructed).any():
                print(f"NaN detected in x_reconstructed at epoch {epoch}")
                continue
            if torch.isnan(adj_reconstructed).any():
                print(f"NaN detected in adj_reconstructed at epoch {epoch}")
                continue
            ...
            for i in range(batch_size_local):
                ...
                ] = dense_adj[i, :num_nodes_in_graph, :num_nodes_in_graph]
                start_idx = end_idx
            ...
            if torch.isnan(grad_norm) or torch.isinf(grad_norm):
                print(f"NaN or Inf gradient detected at epoch {epoch}, skipping batch")
                optimizer.zero_grad()
                continue
        ...
        outer_bar.set_postfix(
            {"Train Loss": f"{avg_train_loss:.6f}", "Val Loss": f"{avg_val_loss:.6f}"}
        )
        ...
        if avg_train_loss < best_loss:
            ...
            best_model = model.state_dict()
            logging.info(
                f"New best model saved with training loss: {best_loss:.6f}"
            )
        else:
            epochs_no_improve += 1
